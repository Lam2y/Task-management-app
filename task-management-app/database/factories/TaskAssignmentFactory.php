...
class TaskAssignmentFactory extends Factory
{
    ...
    public function definition(): array
    {
        ...
        $admin = User::where('usertype', 'admin')->pluck('name')->toArray();
        return [
            'Task' => $this->faker->sentence(),
            'Description' => $this->faker->paragraph(),
            'Assigned_To' => $this->faker->randomElement($employees),
            'Date_Given' => $this->faker->date(),
            'Due_Date' => $this->faker->date(),
            'Priority' => $this->faker->randomElement(['High', 'Medium', 'Low']),
            'Status' => $this->faker->randomElement(['Assigned', 'In progress', 'Completed','Late','Cancelled']),
            'Assigned_By' =>$this->faker->randomElement($admin),
            'Assigned_By_Usertype' => 'admin'
        ];
    }
}
